...
def suggestion_toggle(suggestion_id):
    with engine.begin() as conn:
        current = conn.execute(
            text(
                "SELECT is_resolved FROM githappens_users.user_suggestions WHERE suggestion_id = :id"
            ),
            {"id": suggestion_id},
        ).fetchone()
        if not current:
        ...
        conn.execute(
            text(
                "UPDATE githappens_users.user_suggestions SET is_resolved = :status WHERE suggestion_id = :id"
            ),
