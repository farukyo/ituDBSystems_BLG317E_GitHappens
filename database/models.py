...
user_liked_series = db.Table('user_liked_series',
    db.Column('user_id', db.Integer, db.ForeignKey('users.id'), primary_key=True),
    db.Column('series_id', db.Integer, db.ForeignKey('series.id'), primary_key=True)
)

user_liked_actors = db.Table('user_liked_actors',
    db.Column('user_id', db.Integer, db.ForeignKey('users.id'), primary_key=True),
...
class User(UserMixin, db.Model):
    ...
    liked_movies = db.relationship('Movie', secondary=user_liked_movies,
                                   lazy='subquery', backref=db.backref('liked_by_users', lazy=True))
    
    liked_series = db.relationship('Series', secondary=user_liked_series,
                                   lazy='subquery', backref=db.backref('liked_by_users', lazy=True))
                                   
    liked_actors = db.relationship('Actor', secondary=user_liked_actors,
                                   lazy='subquery', backref=db.backref('liked_by_users', lazy=True))
